...
//!
//! Downloads Proton-GE / Wine-GE releases, verifies SHA256 against the
...
impl RunnerModule {
    ...
    pub fn execute(&self, action: &RunnerAction) -> Result<PathBuf, PlayError> {
        match action {
            ...
            RunnerAction::Download {
                ...
                // Verify SHA256
                self.verify_checksum(&archive_path, sha256)?;
    ...
    fn download_once(&self, url: &str, dest: &Path) -> Result<(), String> {
    ...
    /// Verify SHA256 checksum of a file against the expected hex digest.
    fn verify_checksum(&self, path: &Path, expected: &str) -> Result<(), PlayError> {
        ...
        if actual.eq_ignore_ascii_case(expected) {
            ...
            Err(PlayError::ChecksumMismatch {
...
mod tests {
    ...
    fn already_installed_returns_path_if_exists() {
        ...
        let action = RunnerAction::AlreadyInstalled {
    ...
    fn install_path_for_uses_convention() {
        ...
        assert_eq!(
            path,
            root.path().join("ProtonGE").join("8.25.0")
        );
    }
